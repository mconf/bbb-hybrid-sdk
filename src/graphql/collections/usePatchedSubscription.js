...
export default function usePatchedSubscription(subscriptionGQL, options) {
  // Prepend `Patched_` to the query operationName to inform
  // the middleware that this subscription support json patch
  // It will also set {fetchPolicy: 'no-cache'} because the cache would not work
  // properly when using json-patch
